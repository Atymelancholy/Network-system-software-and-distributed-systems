...
public final class UdpFastFileClient {
    ...
    // Sliding window for UPLOAD (sender side)
    ...
    private static final byte T_NACK     = 5; // legacy (не используем в новой схеме)
    ...
    // re-used receive buffer (no per-call allocations)
    ...
    public UdpFastFileClient(InetSocketAddress server) throws Exception {
        ...
        // "connected" UDP: OS will filter packets from other peers and can raise ICMP errors
        this.sock.connect(server);

        // small timeout for event-loop style
    ...
    // ======================================================================
    // DOWNLOAD: client is RECEIVER -> sends ACK(base+SACK), server retransmits
    // ======================================================================
    public void download(String remoteName, File localFile, int chunk) throws Exception {
        ...
        try (RandomAccessFile raf = new RandomAccessFile(localFile, "rw")) {
            ...
            while (true) {
                ...
                if (p != null) {
                    ...
                    if (p.type == T_DATA) {
                        ...
                        if (seq >= 0 && seq < totalChunks && !received.get(seq)) {
                            ...
                            if (p.payload.length != expected) continue; // защита
                ...
                // send ACK periodically (not for every packet)
                ...
                // if server said DONE and we are idle, it's ok to stop and let retransmits fix via ACK
                // BUT better keep looping until all received; server will retransmit based on ACK gaps.
                // (So no early break on DONE)
            }
        }

        // tell server we are done (server will close transfer)
    ...
    // ======================================================================
    // UPLOAD: client is SENDER -> sliding window + retransmit + waits ACK
    // ======================================================================
    public void upload(File localFile, String remoteName, int chunk) throws Exception {
        ...
        try (RandomAccessFile raf = new RandomAccessFile(localFile, "r")) {
            while (true) {
                ...
                // 1) send new packets within window
                ...
                // 2) receive packets (ACK/DONE/ERR)
                Packet p = recvPacket();
                if (p != null) {
                    if (p.transferId != tid) {
                        // ignore чужие
                        ...
                        if (p.type == T_ACK) {
                            ...
                            // server says it has everything; we still rely on our all-acked check
                        } else if (p.type == T_ERR) {
                            throw new IOException(p.payloadText());
                        }
                    }
                ...
                // 4) finish if all acked
                if (acked.nextClearBit(0) >= totalChunks) {
                    break;
                }
            }
        }
    ...
    // ======================================================================
    // ACK payload format:
    //   base(int) + [start(int), len(int)] * N   where ranges mean RECEIVED (SACK)
    // We build it from BitSet received.
    // ======================================================================
    private static byte[] buildAckPayload(BitSet received, int totalChunks, int maxRanges) {
        // base = first missing (cumulative)
    ...
    // ranges of RECEIVED after base: [start,len]...
    ...
    private Packet recvPacket() throws IOException {
        try {
            ...
            // REJECT / ICMP Unreachable when connected()
